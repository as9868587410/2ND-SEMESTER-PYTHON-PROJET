...
def update_daily_schedule(route, mode):
    ...
    for i in range(100):  # handles up to 100 past days
        old_day = today - timedelta(days=i)
        old_file = f"{route}_{mode}_{old_day.strftime('%Y-%m-%d')}.txt"
        if os.path.exists(old_file):
            os.remove(old_file)

    # Copy from base file and recreate future 60 days files
    ...
    for i in range(61):  # today + 60 days
        ...
        new_file = f"{route}_{mode}_{new_day.strftime('%Y-%m-%d')}.txt"
        with open(new_file, "w") as f:
            f.writelines(base_data)

    return

def delete_60_days_files(route, mode):
    # Old-style function — no longer needed with updated logic
...
def daily_run():
    ...
    for route in routes:
        for mode in ("car", "train", "bus", "flight"):
            update_daily_schedule(route, mode)
